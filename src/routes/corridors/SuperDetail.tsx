...
export function SuperDetail() {
  ...
  const { additional, annualSaving } = superTopUpPotentialSaving(
    inputs,
    derived,
  );
  const utilisation = derived.totalConcessionalContributions / CONCESSIONAL_CAP;
  const shortHorizon =
  ...
  return (
    ...
    >
      ...
      <Section title="The math">
        <div>
          <MathRow
            label="Concessional cap (2025-26)"
            value={<Money value={CONCESSIONAL_CAP} />}
          />
          <MathRow
          ...
          {inputs.deductiblePersonalSuperContributions > 0 && (
            <MathRow
              ...
              value={
                <Money value={inputs.deductiblePersonalSuperContributions} />
              }
            />
          )}
          <MathRow
      ...
      <Section title="What the tax system is steering">
        <p>
          Concessional contributions are taxed at 15% on the way in, instead of
          your marginal rate. For a PBI worker on ~$90k, that's a
          ~17c-in-the-dollar saving — the largest headline corridor after
          packaging, but the money is locked until preservation age (usually
          60).
        </p>
      </Section>
      ...
      <Section title="Next actions">
        ...
        {additional > 0 && (
          <ul className="list-disc list-inside space-y-1 text-sm">
            <li>
              Ask payroll about setting up concessional (pre-tax) salary
              sacrifice. Any amount from <Money value={1000} />
              /yr up is fine — start small if unsure.
            </li>
            <li>
              Alternative: personal deductible contributions before 30 June,
              with a notice-of-intent-to-claim lodged with your super fund.
            </li>
            <li>
              Watch the unused-cap carry-forward rule: unused cap from the last
              5 years can be used if your super balance was under $500k on 30
              June of the prior FY.
            </li>
          </ul>
        )}
        {additional === 0 && (
          <p>
            You're at the concessional cap. Going over triggers
            excess-contributions tax — worth confirming with your fund.
          </p>
        )}
      </Section>
      ...
      <Section title="Gotchas">
        <ul className="list-disc list-inside space-y-1 text-sm text-stone-700">
          ...
          <li>
            Division 293 adds an extra 15% contributions tax when your total
            Division 293 income exceeds $250k. Shouldn't bite at V1's target
            income range, but watch for it if multiple income streams land in
            the same year.
          </li>
          <li>
            The cap is per person per FY — if you change jobs mid-year, both
            employers' SG still counts.
          </li>
        </ul>
      </Section>
    </CorridorFrame>
