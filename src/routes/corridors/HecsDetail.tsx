...
export function HecsDetail() {
  ...
  if (!inputs.hasHECS) {
    return (
      <CorridorFrame title="HECS / STSL" status={summary.status} headline={summary.headline}>
        <Section title="Not applicable">
          <p>
            You didn't flag a HECS/HELP/STSL debt, so this corridor doesn't apply. If
            you do have a debt, go back to inputs and toggle it on.
          </p>
        </Section>
      </CorridorFrame>
    );
    ...
    <CorridorFrame title="HECS / STSL" status={summary.status} headline={summary.headline}>
      <Section title="Your repayment income">
        <div>
          ...
          {inputs.netFinancialInvestmentLosses + inputs.netRentalPropertyLosses > 0 && (
            <MathRow
              label="+ Net investment losses"
              value={
                <Money
                  value={inputs.netFinancialInvestmentLosses + inputs.netRentalPropertyLosses}
                />
              }
            />
          )}
      ...
      <Section title="2025-26 repayment schedule (marginal)">
        ...
        <p className="text-xs text-stone-500 mt-2">
          "Marginal" means the rate applies only to the income above each threshold, not
          to your whole repayment income. This is the regime from 2025-26 onward.
        </p>
      </Section>
      ...
      <Section title="What the tax system is steering">
        <p>
          ...
          mortgage application and the servicing calculator treats HELP as an ongoing
          commitment.
        </p>
      </Section>
      ...
      <Section title="Gotchas">
        <ul className="list-disc list-inside space-y-1 text-sm text-stone-700">
          <li>
            Salary packaging inflates your HECS repayment income. Packaging $15,900
            adds ~$30,000 to the income used to calculate your compulsory repayment —
            which can push you into a higher marginal repayment bracket.
          </li>
          <li>
            The balance indexes each 1 June using CPI (capped at WPI under the 2024
            reforms). Indexation isn't interest — you can't deduct it.
          </li>
          <li>
            Your employer withholds based on estimated income; any mismatch is settled
            at tax time.
          </li>
        </ul>
      </Section>
