...
export function PackagingGeneralDetail() {
  ...
  const { additional, annualSaving } = packagingPotentialSaving(inputs, derived);
  ...
  return (
    ...
    >
      ...
      <Section title="What the tax system is steering">
        <p>
          Salary packaging is a deliberate perk. PBIs and public hospitals can't pay
          market wages, so the government lets them sacrifice salary into FBT-exempt
          benefits (mortgage/rent payments, credit card bills). It effectively raises
          your after-tax pay by the marginal rate on the packaged amount.
        </p>
        <p>
          PBI cap is <Money value={PBI_GENERAL_CAP} />/yr. Public hospital cap is{' '}
          <Money value={PUBLIC_HOSPITAL_GENERAL_CAP} />. The gross-up factor{' '}
          <span className="font-mono tabular-nums">× {PBI_GROSSUP_FACTOR}</span> converts
          packaged dollars into a "grossed-up" taxable amount that appears as RFBA on
      ...
      <Section title="Next actions">
        {additional > 0 && (
          <ul className="list-disc list-inside space-y-1 text-sm">
            <li>
              Talk to your payroll provider or packaging administrator (Maxxia, Smart,
              AccessPay, etc.) about lifting your packaged amount by{' '}
              <Money value={additional} round10 />.
            </li>
            <li>
              The saving is approximate — packaging pushes you down a tax bracket in
              some cases, which changes the marginal rate. The{' '}
              <Money value={annualSaving} round10 /> figure is a lower-bound ballpark.
            </li>
            <li>
              Know what you're packaging into. Rent/mortgage/general card are the most
              common. Some providers charge admin fees; the fee is usually trivial
        ...
        {derived.generalPackagingCap === 0 && (
          <p>
            Your employer type doesn't offer FBT-exempt packaging. If you're unsure,
            ask HR — sometimes an NFP qualifies as a PBI without the employer marketing
            it strongly.
          </p>
        )}
      </Section>
      ...
      <Section title="Gotchas">
        <ul className="list-disc list-inside space-y-1 text-sm text-stone-700">
          <li>
            ...
            <em>HECS repayment income</em> — check both corridors before increasing
            your packaging.
          </li>
          <li>
            Packaging doesn't reduce your Super Guarantee base unless your employer
            calculates SG on post-packaged wages. Most PBIs don't; confirm with
            payroll.
          </li>
          <li>
