...
describe('marginalTaxRate', () => {
  ...
  it('is 0.30 in the middle band', () => {
    expect(marginalTaxRate(100_000)).toBe(0.30);
  });
...
describe('deriveValues — MLS triggers when MLS income crosses threshold', () => {
  ...
  it('crosses into Tier2 at $120k', () => {
    const d2 = deriveValues(baseInputs({ grossAnnualSalary: 120_000 }), PRE_APRIL_2026);
  ...
  it('private cover zeroes the MLS liability', () => {
    const withCover = deriveValues(
      baseInputs({ grossAnnualSalary: 120_000, hasPrivateHospitalCover: true, privateHealthPremiumAnnual: 1_500 }),
      PRE_APRIL_2026,
    );
...
describe('PHI rebate picks the rate for the date', () => {
  it('uses pre-April 2026 base rate for an October 2025 calc', () => {
    const d = deriveValues(
      baseInputs({ hasPrivateHospitalCover: true, privateHealthPremiumAnnual: 2_000 }),
      PRE_APRIL_2026,
    );
...
describe('age-based discount applies to under-30s', () => {
  it('is 10% at age 25', () => {
    ...
    expect(d.ageBasedDiscount).toBe(0.10);
  });
  it('is 0% at age 30+', () => {
...
describe('net PHI cost applies both ABD and rebate', () => {
  it('combines age discount and rebate', () => {
    const d = deriveValues(
      baseInputs({
        age: 25,
        hasPrivateHospitalCover: true,
        privateHealthPremiumAnnual: 2_000,
      }),
      PRE_APRIL_2026,
    );
    // 2000 × 0.9 × (1 − 0.24288) = 2000 × 0.9 × 0.75712 = 1362.816
    expect(d.netPhiCostAnnual).toBe(Math.round(2000 * 0.9 * (1 - 0.24288)));
  });
});
