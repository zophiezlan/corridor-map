...
export function mlsCorridor(
  ...
  const { mlsTier, mlsLiabilityAnnual, netPhiCostAnnual, breakEvenPremiumAnnual } = derived;
  const tierRateMap = { Base: 0, Tier1: 0.01, Tier2: 0.0125, Tier3: 0.015 } as const;
  ...
  if (inputs.hasPrivateHospitalCover) {
    ...
    let insight: string;
    if (mlsTier === 'Base') {
      insight = 'You have cover, but your income is below the MLS threshold — MLS doesn\'t apply to you.';
    ...
    return {
      ...
      status: 'green',
      headline: netCost > 0 ? `${formatAUD(netCost, { round10: true })}/yr net premium` : 'Covered',
      insight,
      applicable: true,
    };
  }
  ...
  if (mlsTier === 'Base') {
    return {
      ...
      insight: 'Your income is below the MLS threshold. Private cover is optional from a tax standpoint.',
      applicable: true,
    };
  }
  ...
  const savings =
    breakEvenPremiumAnnual != null
      ? `A compliant hospital policy up to ~${formatAUD(breakEvenPremiumAnnual, { round10: true })}/yr would break even.`
      : '';
  return {
    id: 'mls',
    name: 'MLS / Private Health',
    ...
    insight: `You're paying MLS that a private hospital policy could avoid. ${savings}`.trim(),
    applicable: true,
  };
}
...
export function superCorridor(
  ...
  const shortHorizon = inputs.propertyGoal === 'within-12m' || inputs.propertyGoal === '1-3y';
...
export function fhssCorridor(inputs: UserInputs): CorridorCardSummary {
  const relevant = inputs.propertyGoal === 'within-12m' || inputs.propertyGoal === '1-3y';
  return {
    id: 'fhss',
...
export function hecsCorridor(
  ...
  if (derived.hecsRepaymentAnnual === 0) {
    return {
      ...
      insight: 'Your repayment income is below the threshold — no compulsory repayment this year.',
      applicable: true,
    };
  }
