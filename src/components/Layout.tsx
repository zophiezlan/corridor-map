...
function BrandMark() {
  return (
    <span
      aria-hidden
      className="inline-flex h-7 w-7 items-center justify-center rounded-md bg-stone-900 text-white"
    >
      <svg
        viewBox="0 0 20 20"
        className="h-4 w-4"
...
export function Layout() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-stone-50 text-stone-900 antialiased">
      ...
      <header className="border-b border-stone-200 bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/70">
        <div className="mx-auto max-w-5xl px-6 py-3 flex items-center justify-between gap-4">
          <Link
            to="/"
            className="flex items-center gap-2.5 font-semibold tracking-tight text-stone-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-stone-700 focus-visible:ring-offset-2 rounded-sm"
            aria-label="Corridor Map — home"
          >
            <BrandMark />
            <span className="text-[15px] leading-none">Corridor Map</span>
            <span
              className="hidden sm:inline-block text-[10px] font-mono uppercase tracking-[0.12em] text-stone-500 ml-1 px-1.5 py-0.5 rounded border border-stone-200"
      ...
      <footer className="border-t border-stone-200 bg-white">
        <div className="mx-auto max-w-5xl px-6 py-6 text-xs text-stone-500 leading-relaxed grid gap-3 sm:grid-cols-[1fr_auto] sm:items-start">
          ...
          <div className="sm:text-right sm:pl-6 sm:border-l sm:border-stone-200 space-y-1">
            <p className="font-mono uppercase tracking-[0.12em] text-[10px] text-stone-400">
              Corridor Map · v1
            </p>
            <p>
              <Link
                to="/about"
                className="text-stone-600 hover:text-stone-900 underline underline-offset-2 decoration-stone-300"
              >
                About &amp; methodology
              </Link>
            </p>
