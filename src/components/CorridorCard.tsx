    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2">
          <span className={`h-2 w-2 rounded-full ${statusDotClass(summary.status)}`} aria-hidden />
          <h3 className="font-semibold text-stone-900">{summary.name}</h3>
        </div>
        <StatusBadge status={summary.status} />
      </div>
      <p className="mt-3 font-mono tabular-nums text-xl text-stone-900">{summary.headline}</p>
      <p className="mt-2 text-sm text-stone-600 leading-snug">{summary.insight}</p>
